...
class S2(BaseWorkFinder):
    ...
    def find_work_list(self):
        ...
        aoi_str = """MULTIPOLYGON (((-175 -12,-179.99999 -12,-179.99999 -20,-175 -20,-175 -12)), ((175 -12,179.99999 -12,179.99999 -20,175 -20,175 -12)))"""
        aoi = loads(aoi_str)
        ...
        # save to file before_processing.csv
        esa_l1c.to_csv("before_processing.csv")
        ...
        logger.info(f"Found {len(esa_l1c)} L1C scenes to process")
        # save esa_l1c to .csv
        # rename scenename column to id
        #  esa_l1c.rename(columns={'scenename': 'id'}, inplace=True)
        esa_l1c.to_csv("esa_l1c.csv")
        logger.info(f"Type of ESAL1C : {type(esa_l1c)}")
        # drop the geometry column
        esa_l1c.drop(columns='geometry', inplace=True)
        esa_l1c = pd.DataFrame(esa_l1c)
        logger.info(f"Type of ESAL1C : {type(esa_l1c)}")
        return esa_l1c
    ...
    def submit_tasks(self, to_do_list: pd.DataFrame):
        ...
        for index, row in to_do_list.iterrows():
            ...
            self._redis.publish(target_queue, json.dumps(row_json))
        exit(1)
